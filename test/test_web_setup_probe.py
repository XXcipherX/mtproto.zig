...
spec = importlib.util.spec_from_file_location("web_probe", ROOT / "deploy/web_probe.py")
...
class ProbeTests(unittest.TestCase):
    ...
    def check_fixture(self, mode, trusted=True, bridge_path="/", ws_path="/custom-socket"):
        ...
        def serve():
            ...
            try:
                connections = 1 if mode in ("static", "trickle") or not trusted else 2
                    ...
                    with server_context.wrap_socket(raw, server_side=True) as conn:
                        ...
                        if index == 0:
                            ...
                            body = b"<html>ordinary website</html>" if mode == "static" else (
                                ...
                                b'"><meta name="tproxy-ws-path" content="' + ws_path.encode() + b'">'
                            )
                        ...
                        body = (struct.pack("<I", 0x05162463) + echoed + b"s" * 16 +
                                b"\x01\x17\0\0" + struct.pack("<IIQ", 0x1CB5C415, 1, 123))
        ...
        try:
            kwargs = dict(address="127.0.0.1", port=port, context=context)
            if mode == "success" and trusted:
                probe.run(data, **kwargs)
            else:
                with self.assertRaises((probe.ProbeError, ssl.SSLError)):
                    probe.run(data, timeout=0.1 if mode == "trickle" else 8, **kwargs)
        finally:
            thread.join(3)
        self.assertFalse(thread.is_alive())
        self.assertEqual(errors, [])
        if trusted:
            self.assertIn("GET " + bridge_path + "?bridge=" + "B" * 43 + " HTTP/1.1", seen[0])
        if mode == "trickle":
            self.assertLess(time.monotonic() - started, 0.5)
        if trusted and mode not in ("static", "trickle"):
            self.assertIn("GET " + ws_path + " HTTP/1.1", seen[1])
    ...
    def test_base_path_variant_uses_both_effective_routes(self):
        self.check_fixture("success", bridge_path="/relay/Path_1/",
                           ws_path="/relay/Path_1/api/v1/socket")
