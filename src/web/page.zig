...
//!
//! The relay supplies a short-lived carrier bearer and CSP nonce. The bearer is sent
//! only as the WebSocket subprotocol and never copied into the WebSocket URL.
...
test "session bridge is standalone and keeps bearer out of websocket URL" {
    const allocator = std.testing.allocator;
    const token = "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT";
    const html = try renderSessionBridge(allocator, "/relay/Path_1/api/v1/socket", token, "nonce_123");
    defer allocator.free(html);

    try std.testing.expect(std.mem.containsAtLeast(u8, html, 1, "<meta name=\"tproxy-token\" content=\"" ++ token ++ "\">"));
    try std.testing.expect(std.mem.containsAtLeast(u8, html, 1, "<meta name=\"tproxy-ws-path\" content=\"/relay/Path_1/api/v1/socket\">"));
...
test "session bridge escapes websocket path for HTML and JavaScript" {
    const allocator = std.testing.allocator;
    const html = try renderSessionBridge(
        allocator,
        "/a<b>c&d\"e",
    ...
    defer allocator.free(html);
    try std.testing.expect(!std.mem.containsAtLeast(u8, html, 1, "<b>"));
    try std.testing.expect(std.mem.containsAtLeast(u8, html, 1, "/a&lt;b&gt;c&amp;d&quot;e"));
    try std.testing.expect(std.mem.containsAtLeast(u8, html, 1, "\\u003c"));
    try std.testing.expect(std.mem.containsAtLeast(u8, html, 1, "\\\""));
}
