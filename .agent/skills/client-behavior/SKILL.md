...
- Keep proxy-side wedge recovery limited to generic DC relays. The fast path is only eligible after established relay traffic resumes from a configured quiet period; media relays and fresh reconnects must not enter it. Do not arm the conservative fallback before 30 seconds in relay, and then require a delivered server reply followed by further client traffic on the same connection. Fresh-relay recovery is a separate cross-slot signal: a matching replacement must already be ready, only the oldest unanswered unproven relay may close, and the source/access-user/DC group is limited to one such close every 120 seconds.
