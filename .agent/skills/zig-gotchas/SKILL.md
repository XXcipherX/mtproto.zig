...
- Runtime discovery runs in a joinable updater thread during startup without blocking on discovery when MiddleProxy or masking resolution is active; shutdown is cooperative, DNS/HTTPS/curl tasks are canceled in their owning thread, and endpoint probes run in cancellable batches of at most four sockets.
- Keep `SIGINT`/`SIGTERM` cleanup out of async signal context. The production handler may only notify the non-blocking `eventfd`; with multiple workers one control thread reads that fd and broadcasts to distinct worker eventfds. Allocator, socket, epoll, listener, and updater cleanup belongs to event-loop/defer paths. The first notification disables listener interest and begins the configured graceful drain; accepts must never resume while `shutting_down`, and another notification or the deadline force-closes remaining slots. The signal bridge must outlive every runtime worker and is dismantled only after the updater is joined. Do not permanently block these signals process-wide for `signalfd`: Zig 0.16's POSIX child-spawn path preserves the caller's signal mask across `exec`, which would also block `std.process` cancellation signals in curl fallback children.
...
- Mask-relay close diagnostics keep the initial classification in `mask_cause`; the later `reason` is only the relay termination. Preserve the distinction between expected `web_carrier`, routing failures, structural FakeTLS failures, secret mismatch, clock skew, and replay. Authentication failures must be classified during the original constant-time secret scan, not by recomputing HMAC for logging. For `timestamp_skew`, retain the original signed `skew_s` calculation (`server - client`) alongside the cause instead of trying to reconstruct it after authentication material has been wiped.
