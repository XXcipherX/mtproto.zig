...
- Runtime relay remains one worker-local `epoll` loop per configured `[server].workers` (default `1`). `0` auto-selects a bounded count; `2..16` runs independent `SO_REUSEPORT` listeners. Total connection slots and managed-buffer budgets remain process-wide, and the separate WEB relay/Caddy deployment is unchanged.
