...
- `ios_wedge: candidates+=... cancelled+=... fresh_close+=... proven_close+=... suppressed+=...` is emitted only when `client_silence_close_sec` is enabled. `fresh_close` is a bounded first-exchange recovery, `proven_close` follows an earlier mature healthy continuation, and `suppressed` counts transitions into a per-client/DC backoff, idle-deadline, or fixed-table fail-safe episode rather than every matching exchange left to ordinary idle timeout. Per-candidate `armed` debug output is emitted once even when continued server delivery moves its deadline.
