...
enable_netfilter_persistent() {
    ...
    for attempt in {1..10}; do
        ...
        if output="$(systemctl enable netfilter-persistent.service 2>&1)"; then
            return 0
        fi
    ...
    [[ -z "$output" ]] || printf '%s\n' "$output" >&2
    fail "Cannot enable firewall restoration; verify netfilter-persistent and systemd"
}
...
# disabled by default because tiny MSS can hurt media throughput. Both legacy
# and current spellings are removed before reapplying so upgrades cannot leave
