...
- Outbound client/upstream writes use intrusive page-sized block queues backed by one capped event-loop block pool, bounded `writev` dispatches, and a 4 MiB pending-byte cap per queue. Tail packing avoids one page allocation per small write, recycled pages are wiped, and per-event byte/operation budgets prevent one ready fd from monopolizing the loop.
