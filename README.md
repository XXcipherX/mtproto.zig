...
| **MiddleProxy** | Telemt-Compatible ME | Optional ME transport for DC1..5 (`use_middle_proxy`); retries all Telegram-provided endpoints before direct fallback when unavailable |
...
| `[server]` | `dc_connect_timeout_sec` | `10` | Per-endpoint TCP connect deadline for Telegram DC and MiddleProxy candidates. If one endpoint is black-holed, the proxy abandons it and tries the next Telegram-provided candidate within the global handshake timeout. Set `0` to disable |
